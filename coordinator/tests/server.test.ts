...
import { NodeRegistry, stableNodeId, type NodeInfo } from "../src/registry.ts";
...
test("POST /generate for a warm pipeline with an active driver never calls a registered launcher, even when one exists for the same model", async () => {
  ...
  try {
    ...
    launcherRegistry.register(launcherStub.endpoint, await canonicalizeEndpoint(launcherStub.endpoint), ["big-model"], launcherPort);
...
test("POST /generate with a non-empty pool never calls ensurePipelineReady's launcher-assembly path", async () => {
  ...
  try {
    ...
    registry.register("http://127.0.0.1:1", await canonicalizeEndpoint("http://127.0.0.1:1"), "desktop");
    registry.register("http://127.0.0.1:2", await canonicalizeEndpoint("http://127.0.0.1:2"), "desktop");
    launcherRegistry.register(launcherStub.endpoint, await canonicalizeEndpoint(launcherStub.endpoint), ["big-model"], Number(new URL(launcherStub.endpoint).port));
...
test("POST /generate falls back to ensurePipelineReady's cold-start assembly when the pool is empty", async () => {
  ...
  try {
    registry.register("http://127.0.0.1:1", await canonicalizeEndpoint("http://127.0.0.1:1"), "desktop");
    registry.register("http://127.0.0.1:2", await canonicalizeEndpoint("http://127.0.0.1:2"), "desktop");
    launcherRegistry.register(stub.endpoint, await canonicalizeEndpoint(stub.endpoint), ["big-model"], Number(new URL(stub.endpoint).port));
...
test("POST /generate's cold-start assembly never claims a launcher already backing another model's pool entry", async () => {
  ...
  try {
    ...
    const launcherId = launcherRegistry.register(sharedLauncher.endpoint, await canonicalizeEndpoint(sharedLauncher.endpoint), ["model-a", "model-b"], agentPort);
...
test("POST /generate's cold-start assembly skips a claimed launcher and uses an unclaimed one serving the same model", async () => {
  ...
  try {
    ...
    const claimedId = launcherRegistry.register(claimedLauncher.endpoint, await canonicalizeEndpoint(claimedLauncher.endpoint), ["model-a", "model-b"], Number(new URL(claimedLauncher.endpoint).port));
    launcherRegistry.register(freeLauncher.endpoint, await canonicalizeEndpoint(freeLauncher.endpoint), ["model-b"], Number(new URL(freeLauncher.endpoint).port));
...
test("two different models cold-starting at once never share one launcher", async () => {
  // Regression test for the gap a re-review found in the FIRST fix for
  // concurrent assembly: the dedup Map is keyed by modelId, so it does not
  // serialise two DIFFERENT models, and the cold-start path recorded
  // nothing until its launcher call returned. Both models therefore saw the
  // single launcher as unclaimed and both claimed it -- and since a
  // swarm-launcher supervises one agent at a time, a user asking for
  // model-a was served model-b's weights with a 200.
  let currentModel = "none";
  const stub = await startStubNodeAgent((body) => {
    const candidate = body as Record<string, unknown>;
    if (candidate.model !== undefined) {
      currentModel = String(candidate.model);
      return { status: 200, body: { status: "ready" } };
    }
    return { status: 200, body: { text: `generated by ${currentModel} weights` } };
  });
  const twoModels = [
    { id: "model-a", displayName: "A", minActiveNodes: 0, requiredNodeCount: 2, maxPipelines: 1 },
    { id: "model-b", displayName: "B", minActiveNodes: 0, requiredNodeCount: 2, maxPipelines: 1 },
  ...
  try {
    ...
    launcherRegistry.register(stub.endpoint, await canonicalizeEndpoint(stub.endpoint), ["model-a", "model-b"], launcherPort);

    await Promise.all(["model-a", "model-b"].map(modelId =>
      authFetch(`${baseUrl}/generate`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ prompt: "hi", modelId }),
      })));
    ...
    assert.equal(new Set(driverIds).size, driverIds.length, "one driverNodeId must never appear in two models' entries");
  } finally {
    server.close();
    stub.server.close();
  }
...
test("POST /generate does not respawn a driver that is already reputation-ejected", async () => {
  // The trust pre-flight was first added only to the background loop. On
  // the request path an ejected driver was respawned on EVERY /generate --
  // a real multi-GB model load and kill per request, higher-frequency than
  // the per-tick loop the guard was written for.
  let launcherCallCount = 0;
  const stub = await startStubNodeAgent((body) => {
    const candidate = body as Record<string, unknown>;
    if (candidate.model !== undefined) {
  ...
  try {
    ...
    try {
      ...
      control.launcherRegistry.register(stub.endpoint, await canonicalizeEndpoint(stub.endpoint), ["big-model"], launcherPort);
