...
int main(int argc, char** argv) {
    ...
    // Endpoint Identity Hardening: what this agent authoritatively reports
    // about itself via POST /identity, so a coordinator verifying a
    // registration asks the agent rather than trusting whoever called
    // register(). servesModel is the catalog id (e.g. "tinyllama-1.1b"),
    // distinct from modelPath above (a file path) -- this agent had no way
    // to report a catalog id at all before this flag existed. deviceTier
    // defaults to "desktop", matching what Phase B/C already hardcode for
    // every launcher-spawned driver.
    std::string servesModel;
    std::string deviceTier = "desktop";
    ...
    try {
        for (int i = 1; i < argc; ++i) {
            ...
            } else if (arg == "--serves-model" && i + 1 < argc) {
                servesModel = argv[++i];
            } else if (arg == "--device-tier" && i + 1 < argc) {
                deviceTier = argv[++i];
            } else {
        ...
        if (modelPath.empty() || port <= 0) {
            std::fprintf(stderr,
                         ...
                         "[--serves-model id] [--device-tier desktop|android|ios]\n",
                         argv[0]);
            return 1;
        }

        if (deviceTier != "desktop" && deviceTier != "android" && deviceTier != "ios") {
            std::fprintf(stderr, "error: --device-tier must be one of: desktop, android, ios (got \"%s\")\n", deviceTier.c_str());
            return 1;
        }
        ...
        // Endpoint Identity Hardening: lets the coordinator verify a
        // registration by asking THIS agent what it serves, rather than
        // trusting whatever the caller of POST /nodes/register claimed. The
        // nonce is single-use and generated by the coordinator per
        // registration attempt -- echoing it back (rather than this route
        // returning a fixed, replayable answer) is what proves this
        // specific response came from a live round-trip to this endpoint,
        // not a cached or forged one. Auth-gated exactly like /health and
        // /complete -- this route reveals no more about the agent than
        // registration already would, but it's still a live network
        // callback and should not be open to an unauthenticated caller.
        server.route("POST", "/identity", [&authToken, &servesModel, &deviceTier](const swarm::HttpRequest& req) {
            if (!isAuthorized(req, authToken)) {
                return swarm::HttpResponse{401, R"({"error":"missing or invalid Authorization header"})"};
            }
            std::string nonce;
            ...
            // A cap, not a real limit on legitimate use (the coordinator's
            // own nonce is a UUID) -- refuses to echo unbounded
            // attacker-controlled input back into a JSON response.
            if (nonce.size() > 200) {
                return swarm::HttpResponse{400, R"({"error":"nonce must not exceed 200 characters"})"};
            }
            // jsonEscapeString on the nonce: it is attacker-influenced input
            // being embedded in a JSON string value, so it must be escaped,
            // not concatenated raw (a nonce containing a literal '"' would
            // otherwise break the response's JSON structure). deviceTier is
            // not escaped -- it was validated at startup against a fixed
            // three-value enum, so it can never contain a character that
            // needs escaping.
            std::string body = R"({"nonce":")" + swarm::jsonEscapeString(nonce) +
                                R"(","deviceTier":")" + deviceTier + R"("})";
            if (!servesModel.empty()) {
                // Insert before the closing brace rather than appending
                // after it, so the result stays one JSON object.
                body.insert(body.size() - 1, R"(,"servesModel":")" + swarm::jsonEscapeString(servesModel) + R"(")");
            }
