...
Design: [`docs/superpowers/specs/2026-08-20-security-hardening-phase-1-auth-design.md`](docs/superpowers/specs/2026-08-20-security-hardening-phase-1-auth-design.md) — read this for Phase 1's full reasoning. Phases 2–4 don't have design docs yet.
...
| 2 | Real safety-classifier ruleset (replacing the zero-rule default `KeywordSafetyClassifier`) | Not started, no design yet |
| 3 | Sybil-resistant reputation (`ReputationTracker` currently has no defense against churning registrations) | Not started, no design yet |
| 4 | Reputation-ranked node selection for `POST /generate` — narrower than Phase B above (that's multi-node pipeline assembly; this only picks the best single already-registered node) | Not started, no design yet |
...
Phases 2–4 are named and ordered but not yet designed — brainstorm each one fresh against the codebase's actual state at that time, the same way every plan in this project gets scoped.
