...
Phase 4 is named and ordered but not yet designed — brainstorm it fresh against the codebase's actual state, the same way every plan in this project gets scoped. It depends on Phase 3 existing (ranking by reputation is only meaningful once reputation can't be freely reset for a given identity), which is now true, but should still weigh the endpoint-aliasing gap above: reputation-ranking a `nodeId` doesn't mean much if an operator can cheaply mint a fresh one for the same physical node.
